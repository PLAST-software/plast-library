        ...
        switch (c)
        {
            case 'Q':
            {
                ...
                *seqId = 0;
                ...
                map<string,Entry>::iterator lookup = _qryIdMap.find (commentBuffer+1);

                *seqId = ' ';

                if (lookup != _qryIdMap.end())
                {
                    _qrySeq.comment = lookup->second.comment.c_str();
                    _qrySeq.index   = lookup->second.index;
                }
                else
                {
                    pair<map<string,Entry>::iterator,bool> ret = _qryIdMap.insert (
                        pair<string,Entry> (
                            seqId,
                            Entry(commentBuffer+1, _nbQrySeq++)
                        )
                    );

                    _qrySeq.comment = ret.first->second.comment.c_str();
                    _qrySeq.index   = ret.first->second.index;
                }
            ...
            case 'S':
            {
                ...
                *seqId = 0;
                ...
                /** We look whether the read id is already known. */
                map<string,Entry>::iterator lookup = _sbjIdMap.find (commentBuffer+1);

                *seqId = ' ';

                if (lookup != _sbjIdMap.end())
                {
                    _sbjSeq.comment = lookup->second.comment.c_str();
                    _sbjSeq.index   = lookup->second.index;
                }
                else
                {
                    pair<map<string,Entry>::iterator,bool> ret = _sbjIdMap.insert (
                        pair<string,Entry> (
                            seqId,
                            Entry(commentBuffer+1, _nbQrySeq++)
                        )
                    ...
                    _sbjSeq.comment = ret.first->second.comment.c_str();
                    _sbjSeq.index   = ret.first->second.index;
                }
...
QueryReorderVisitor::~QueryReorderVisitor ()
{
    ...
    /** We finalize the process, ie. we are going to reorder the results by query. */
    finalize ();
...
void QueryReorderVisitor::finalize (void)
{
    ...
    FileLineIterator lineIt (getTmpFileUri().c_str(), 1024);
    ...
    for (size_t j=0; j<qryOffsets.size()-1; j++)
    {
        ...
        /** We create the query database for the given range. */
        ...
        for (seqIterator->first(); !seqIterator->isDone(); seqIterator->next ())
        {
            ...
            if (alignStream.getAlignmentsNumber() >= _nbAlignmentsThreshold)
            {
                /** We get the alignments container. */
                IAlignmentContainer* container = alignStream.getContainer();

                /** We makes it accept the final visitor. */
                if (container)  { container->accept (_finalVisitor); }
