    ...
    : _encoding(encoding), _N(N), _matrix(0), _matrixAsVector(0), _defaultScore(defaultScore)
{
    ...
    _matrix = (int8_t**) MemoryAllocator::singleton().calloc (N, sizeof (int8_t*));
    for (size_t i=0;  i<N;  i++)   {  _matrix[i] = & (tmp[i*N]);  }

    /** We allocate the matrix as vector. */
    _matrixAsVector = (int8_t*) os::MemoryAllocator::singleton().calloc (32*32, sizeof(int8_t));
...
ScoreMatrix::~ScoreMatrix ()
{
    ...
    if (_matrix != 0)
    {
        MemoryAllocator::singleton().free (_matrix[0]);
        MemoryAllocator::singleton().free (_matrix);
    }

    MemoryAllocator::singleton().free (_matrixAsVector);
}
